...
// compiler size checks Ka
#if MICROCONTROLLER_COMPILE_TYPE
	#if (CHAR_MAX !=  0xff) || (UCHAR_MAX != 0xff) || (SCHAR_MAX != 0x7f)
		#error "ERROR Size mismatch ->check this compiler!"
	#endif
	#if (SHRT_MAX != 0x7fff) || (USHRT_MAX != 0xffff)
		#error "ERROR Size mismatch ->check this compiler!"
	#endif
...
#if QT_COMPILATION
	#define CPP_ENUM_SIZE_8Bit  : unsigned char
	#define CPP_ENUM_SIZE_16Bit : unsigned short
#else
	#define CPP_ENUM_SIZE_8Bit
	#define CPP_ENUM_SIZE_16Bit
#endif
...
#if MICROCONTROLLER_COMPILE_TYPE   // Uebersetzung fuer MICROCONTROLLER - fuer uip-Stack
	#define	_PTR			void * 	  // fuer memcmp()
